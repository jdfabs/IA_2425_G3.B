...
class Cell:
    ...
    def printBorders(self):
        print(self.top_border.hasWall())
        print(self.bottom_border.hasWall())
        print(self.left_border.hasWall())
        print(self.right_border.hasWall())
...
class Board:
    ...
    def displayBoard(self):
        ...
        for x in range(size):
            ...
            for y in range(size):
                top_border += "+"
                if self.getCell(x, y).top_border.hasWall():
            ...
            row_content = ""
            for y in range(size):
                if self.getCell(x, y).left_border.hasWall():
            ...
            if self.getCell(x, size - 1).right_border.hasWall():
                row_content += "|"
            else:
                row_content += " "
        ...
        for y in range(size):
            bottom_border += "+"
            if self.getCell(size - 1, y).bottom_border.hasWall():
                bottom_border += "---"
            else:
                bottom_border += "   "
        bottom_border += "+"
